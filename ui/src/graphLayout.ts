...
export type MnemonicCount = {
  mnemonic: string;
  count: number;
};

export type GraphNodeDetails = {
  directInputs?: ArtifactSummary[];
  directOutputs?: ArtifactSummary[];
  mnemonics?: MnemonicCount[];
};
...
export type GraphNode = {
  ...
  outputBytes?: number;
  actionCount?: number;
  mnemonicSummary?: MnemonicCount[];
  topFiles?: ArtifactSummary[];
  topOutputs?: ArtifactSummary[];
...
function isRuleLike(node: Pick<GraphNode, 'nodeType'>): boolean {
  ...
  pg.nodes.forEach((node) => {
    switch (mode) {
      ...
      case 'source-file-count':
        node.weight = node.sourceFileCount ?? 0;
        break;
      case 'source-bytes':
        node.weight = (node.sourceBytes ?? 0) / 1024;
        break;
      case 'input-file-count':
        node.weight = node.inputFileCount ?? 0;
        break;
      case 'input-bytes':
        node.weight = (node.inputBytes ?? 0) / 1024;
        break;
      case 'output-file-count':
        node.weight = node.outputFileCount ?? 0;
        break;
      case 'output-bytes':
        node.weight = (node.outputBytes ?? 0) / 1024;
        break;
      case 'action-count':
        node.weight = node.actionCount ?? 0;
        break;
      case 'uniform':
        node.weight = 1;
        break;
...
export function sanitizeGraph(raw: Graph): Graph {
  ...
  for (const n of raw.nodes) {
    if (isValidId(n.id)) {
      nodeMap.set(n.id, { ...n, id: n.id, label: n.label || n.id });
    }
  }
...
function buildPositionedGraph(
  ...
  const hotspotNodeCount = nodes.filter((n) => isRuleLike(n) && n.isHotspot).length;
