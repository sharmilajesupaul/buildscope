...
# Default to the enriched Angular app fixture if no argument is provided
GRAPH_PATH="${1:-fixtures/buildscope_large_angular_app.json}"
GRAPH_PATH="${GRAPH_PATH/#\~/$HOME}"

if [[ "$GRAPH_PATH" != /* ]]; then
  GRAPH_PATH="$REPO_DIR/$GRAPH_PATH"
fi

ensure_file_exists "$GRAPH_PATH"
...
start_go_server() {
  cd "$REPO_DIR/cli"
  go run ./cmd/buildscope view "$GRAPH_ARG" --addr ":$GO_PORT" &> "$server_log_file" &
  GO_PID=$!
  cd "$REPO_DIR"
...
cd "$REPO_DIR/ui"
run_with_repo_node npm run dev -- --port "$VITE_PORT" &> "$vite_log_file" &
VITE_PID=$!
cd "$REPO_DIR"
